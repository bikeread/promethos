...
`set-agent-autonomy-boundaries` and `design-agent-permissions`, then mixes
...
- evidence available:
  ...
  - bootstrap ordering guidance from `route-agent-design`
...
- `set-agent-autonomy-boundaries` owns checkpoint policy.
- `design-agent-permissions` explicitly assumes autonomy policy is already chosen.
...
**Chosen intervention:**
- Tighten the trigger examples and scope guard around `design-agent-permissions`.
...
`evolve-skill-library` if the fix suggests a library-level boundary change, or
`turn-postmortem-to-improvement` if the incident should become a reusable eval case.
...
system under debug. This keeps `debug-agent-failures` anchored to
